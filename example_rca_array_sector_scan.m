...
%
% This example shows how a RCA transducer acquire RF data in FMC mode and sector scanning. The row elements are arranged
% along the y-axis, while the column elements are arranged along x-axis. The RCA array consists of 256 rows and 32
% columns.
% The RF data is an array with a shape of [Nt, Nv, Nu, Ns], where
% Nt is the temporal sampling length in each emission cycle,
% Nv is the number of reception elements,
% Nu is the number of emission cycles,
% Ns is the sector scanning lines.
% For a RCA array with Nr row elements and Nc column elements, Nv = max(Nr, Nc), Nu = Nr + Nc.
%
...
% Revised by Zhixuan Chang, Version 2.2, January 4, 2026.
...
% 阵列包含8个RCA子阵列，每一个子阵列包含32个行阵元和32个列阵元
% TODO: 对比使用8个32+32孔径和1个32+256孔径在成像质量上的区别
num_rows        = 32;
num_cols        = 32;
num_aper        = 8;

% 最大成像距离设置为1.5m，每一次成像的视场宽度为32mm，对应的扫查步进角度为
fov_depth       = 1.5;
angle_step      = num_rows * pitch / fov_depth; % 0.0256 rad，1.467 deg。扫查范围设定为120 deg，步进角度可以设置为1 deg。
...
%% Set emisstion aperture
delta_x         = c/f0/5;                       % maximum size of a single patch of an element
height          = num_rows*pitch-(pitch-width);  % Height of element [m]
